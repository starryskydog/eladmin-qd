      ...
      >
        ...
        >
          <template slot-scope="scope">
            <span
              v-if="v.field!='productName'&&v.field!='productCategoryName'&&v.field!='productSeriesName'"
            >
              <el-input
                size="mini"
                placeholder="请输入内容"
                ...
                :disabled="v.disabled"
                @focus="handleFocus(v.field)"
              ></el-input>
            </span>
            <span v-else>{{master_user.data[scope.$index][v.field]}}</span>
          </template>
        </el-table-column>
  ...
  methods: {
    setContact(data, index, field) {
      ...
      row.totalNumber =
        ...
        Number(row.jcNumber) +
        Number(row.hjNumber) +
        Number(row.bjcNumber) +
        Number(row.cmNumber) +
        Number(row.jmNumber) +
        Number(row.dphNumber);
    },
    handleSet(data) {
    ...
    handleSave(index) {
      edit(this.master_user.data[index]).then(res => {
        this.key.splice(
          this.key.findIndex(item => item === index),
          1
        );
        this.$notify({
          title: "修改成功",
